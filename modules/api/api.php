...
class api extends bayesComponent {
    ...
    */
    function __construct() {
        ...
        // Declare objects
        //$this->nb = new bayesUser(); // Bayes  (2.06)
				       ...
				       $items = new items();
							 ...
							 if($username!='') {
	    						 ...
	    						 foreach($URLparams as $key => $val) $$key = $val;
									 ...
									 $URLparams = array();
									 $categories = '';
									 $chDescrip = 'Use Case: Return the RSS Feeds for a User. User Nickname: '.$username; //.'. Summary of applied filters: '.substr($filters,0,-2);
									 
                   $rss = new suxRSS();
    							 $rss->outputRSS(ucfirst($username)."'s RSS Feeds$channelStr", 'http://icbl.macs.hw.ac.uk/sux0r206/user/profile/'.$username, $chDescrip, $categories);
    							 $selfURL = 'http://icbl.macs.hw.ac.uk/sux0rAPI/icbl/'.$_GET['c'].$urlParam;
									 $array = array();
									 $userFeedItems = 'http://icbl.macs.hw.ac.uk/sux0rAPI/icbl/api/items/?user='.$username.'&feed_id=';
									 foreach($results as $arr) {
											$thisFeedItems = $arr['rss_feeds_id'];
											$relevance = '';
											$thisFeed = $rss->getFeedByID($arr['rss_feeds_id']);
											if(!(in_array($thisFeed,$array))) {
											   $array[] = $thisFeed;
												 $rss->addOutputItem($thisFeed['title'], $thisFeed['url'], $thisFeed['body_html'],'','','', $thisFeedItems);	
									    }
									 }
				       ...
				       $items = new items();
							 ...
							 if($username!='') {
	    						 ...
	    						 foreach($URLparams as $key => $val) {
									 ...
									 if(sizeof($arrayCats)==0 && ($vec_id>0 || $cat_id>0)) $filters.= ' (invalid given categories were not used); ';
                   $thisFeed = '';
									 $chDescrip = 'Use Case: Return the RSS Items for a User. User Nickname: '.$username.'. Summary of applied filters: '.substr($filters,0,-2);
									 $rss = new suxRSS();
									 if($feed_id>0) {
									   $thisFeed = $rss->getFeedByID($feed_id);
									   if(sizeof($thisFeed)>0) $channelStr = str_replace($feed_id,'(feed ID: '.$feed_id.')',$channelStr.' '.$thisFeed['title']);
                   }
    							 ...
    							 $selfURL = 'http://icbl.macs.hw.ac.uk/sux0rAPI/icbl/'.$_GET['c'].$urlParam;
									 ...
									 if($feed_id>0) { 
									    foreach($results as $arr) {
											...
											if($nofeeds) {
											   //print_r($results);
												 $rss->addOutputItem('No items found', '', 'The provided feed ID didn\'t match any valid feed of '.$username,'','','', '');	
											}
									 } else {
									    foreach($results as $arr) {
											   $relevance = '';
