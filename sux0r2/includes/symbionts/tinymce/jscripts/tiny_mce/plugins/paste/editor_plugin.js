...
(function() {
	...
	tinymce.create('tinymce.plugins.PastePlugin', {
		...
		_insertText : function(content, bLinebreaks) {
			// Updated by Speednet (June 3, 2008)
			content = this.editor.dom.encode(content);

			if (content && content.length > 0) {
				
				// Delete any highlighted text before pasting
				if (this.editor.selection.getContent().length > 0) {
				...
				if (bLinebreaks) {
					...
					if (this.editor.getParam("paste_create_paragraphs", true)) {
						var pos, rpos, rl = this.editor.getParam("paste_replace_list", '\u2122,<sup>TM</sup>,\u2026,...,\x93|\x94|\u201c|\u201d,",\x60|\x91|\x92|\u2018|\u2019,\',\u2013|\u2014|\u2015|\u2212,-').split(',');
						...
						// Has more than one paragraph; pos is the position between the 1st and 2nd paragraphs
						if ((pos = content.indexOf('</p><p>')) != -1) {
							rpos = content.lastIndexOf('</p><p>');
							...
							if (breakElms.length > 0) {
								var before = content.substring(0, pos), after = "";
								
								for (var i=0, l=breakElms.length; i<l; i++) {
									before += "</" + breakElms[i].nodeName.toLowerCase() + ">";
									after += "<" + breakElms[breakElms.length-i-1].nodeName.toLowerCase() + ">";
								}
								
								if (pos == rpos) {
									content = before + after + content.substring(pos+7);
								}
								else {
									content = before + content.substring(pos+4, rpos+4) + after + content.substring(rpos+7);
								}
							}
						}
					}
					else if (this.editor.getParam("paste_create_linebreaks", true)) {
