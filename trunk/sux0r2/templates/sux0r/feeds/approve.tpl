...
<input type="hidden" name="token" value="{$token}" />
    ...
    {validate id="feeds" message=$r->gtext.form_problem}
    {if $r->arr.feeds}
        {foreach from=$r->arr.feeds item=foo}
            ...
            <div class="approveItemOptions">
                <a href="{$r->makeUrl('/feeds/edit')}/{$foo.id}">{$r->gtext.edit}</a> | <a href="{$foo.url}" target="_blank">{$r->gtext.url}</a> |
                <input type="radio" name="feeds[{$foo.id}]" id="f_k_{$foo.id}" value="1" /><label for="f_k_{$foo.id}" > {$r->gtext.approve_2}</label> |
                <input type="radio" name="feeds[{$foo.id}]" id="f_d_{$foo.id}" value="0" /><label for="f_d_{$foo.id}" > {$r->gtext.delete}</label> |
