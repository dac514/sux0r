    ...
    */
    public function addOutputItem($title, $link, $description, $pubDate='', $source='', $relevance='', $url = '') {
		    ...
		    if(!isset($relevance)) $relevance='';
				...
				if(!isset($url)) $url='';
				if(!isset($guidElement)) $guidElement='';
				if(!isset($guidAttrib)) $guidAttrib='';
        ...
        $item->appendChild($this->createElement('link', htmlspecialchars($link, ENT_QUOTES, 'UTF-8', false)));
				$guid = $link;
				if($source=='' && $url!='') $guid = $url;
				$guidElement = $item->appendChild($this->createElement('guid', htmlspecialchars($guid, ENT_QUOTES, 'UTF-8', false)));
        $guidElement->setAttribute("isPermaLink", "false");
				$item->appendChild($this->createElement('description', str_replace('&nbsp;','&#160;',htmlspecialchars($description, ENT_QUOTES, 'UTF-8', false))));
